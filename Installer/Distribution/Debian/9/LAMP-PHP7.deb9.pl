...
until ($quit) {
	print "Should we install LAMP for you?  \n";
    print "Enter Y|N|Q: ";
    ...
    if ($input =~ /^[Y]?$/i) {      # Match Yy or blank
		...
		system ("apt install -y php7.0-geoip php7.0-intl php7.0-json php7.0-mbstring php7.0-mcrypt php7.0-mysql php7.0-opcache php7.0-readline php7.0-xml php7.0-xsl php7.0-zip \n");
		system ("clear");
		...
		sleep 2;
		system ("apt install php7.0-apcu -y");
		system ("service apache2 restart");
		system ("clear");
		printf ("Installing Mysql Server\n");
		sleep 2;
		system ("apt -y install mariadb-server mariadb-client");
		system ("clear");
		printf ("Installing PHPMyadmin\n");
		sleep 2;
		system ("apt -y install phpmyadmin");
		system ("clear");
